...
|(l) When pages utilize scripting languages to display content, or to create interface elements, the information provided by the script shall be identified with functional text that can be read by Assistive Technology.|Supports with exceptions|Navigation which uses scripting is accessible to assistive technologies and the keyboard.<br>Some share functionality - and the recommendations service - are only available when utilizing scripting.<br>We are moving towards supporting fully.|
