...
| [Section 1194.41 Information, Documentation and Support](#section-119441-information-documentation-and-support) |  |  |
...
| (c) Web pages shall be designed so that all information conveyed with color is also available without color, for example from context or markup. | Supports | Website is still readable with stylesheet disabled and without loss of functionality |
...
| (h) Markup shall be used to associate data cells and header cells for data tables that have two or more logical levels of row or column headers. | Does not currently support | Working towards supporting
HTML provided by third party may contain data tables without proper markup. Efforts are made to encourage proper table markup at the data ingestion stage.
We use Pa11y to try and catch specific markup issues and then correct them at source. |
